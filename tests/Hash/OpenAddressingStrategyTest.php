...
use DataStructures\Hash\OpenAddressingStrategy;
use PHPUnit\Framework\TestCase;

class OpenAddressingStrategyTest extends TestCase {
    private $hashmap;

    // Constructor tests
    public function testFailLoadFactorHigherThan1() {
        $this->expectException(\OutOfBoundsException::class);
        $this->hashmap = new OpenAddressingStrategy(5, 1.2);
    }

    public function testFailLoadFactorLowerOrEqualTo0() {
        $this->expectException(\OutOfBoundsException::class);
        $this->hashmap = new OpenAddressingStrategy(5, 0);
    }

    public function testFailResizeFactor() {
        $this->expectException(\OutOfBoundsException::class);
        $this->hashmap = new OpenAddressingStrategy(5, 0, 1);
    }

    // Getters tests
    public function testGetLoadFactor() {
        $this->hashmap = new OpenAddressingStrategy();
        $this->assertEquals($this->hashmap->getLoadFactor(), 0.75);
        $this->hashmap = new OpenAddressingStrategy(5, 0.5);
        $this->assertEquals($this->hashmap->getLoadFactor(), 0.5);
    }

    // Functionality methods tests
    public function testSearch() {
        $this->hashmap = new OpenAddressingStrategy(5);
    }
    
    public function testInsert() {
        $this->hashmap = new OpenAddressingStrategy(2);
